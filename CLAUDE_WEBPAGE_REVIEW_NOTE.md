...
# 連線對戰 3–4 人修復（2026-07-10 追加）

底層（`Net.conns[]` 多連線、對戰中 `sendExcept` 轉播）本來就支援 4 人，但大廳配對有兩個對 3–4 人才會炸的 bug，已修：

## Bug A：族群撞號只比對房主 → 已修
- 原本 `if (guestTribe === L.tribe)` 只擋「跟房主同族」，訪客彼此同族不會擋 → 3–4 人可能兩人同族。
- 改成比對**所有已入座玩家**的族，撞號自動改派到還沒被選走的族（4 族剛好夠 4 人各一）。

## Bug B：訪客座位用名字比對 → 已修
- 原本 `myIdx = findIndex(p => p.name === myName)`，同名／沒填名字 → 兩台裝置以為自己是同一個玩家索引。
- 改成**房主在 join 時直接指派穩定座位**（`{t:'assign', idx}`），訪客用收到的 idx，不再靠名字猜。留舊 findIndex 當回退。

## UX：等待畫面加入座進度 → 已加
- 房主／訪客大廳都顯示入座名單（頭像＋名字＋族＋「房主」標）與「X / Y」進度，`netRosterHTML()`。

## 驗證（真・端到端，同頁開 4 條 PeerJS 連線 + WebRTC）
房主邵族；訪客A 選邵族→改派噶瑪蘭(idx1)；訪客B 選賽德克→保留(idx2)；訪客C 選賽德克(撞B)→改派拉阿魯哇(idx3)。4 族不重複、座位 0/1/2/3 不重複、滿員自動開局。
