...
## A18：連線對戰補上雙向互動（猜拳／交易，2026-07-10 JJ 指示）

**背景**：A17 連線 v1 把 5 個對抗行動停用。JJ 要求把「猜拳／互動」在連線版修好。

**架構**：發起方（我方回合）在本地跑互動，需要對方輸入時透過 net 請求，對方在自己畫面回應；解出最終結果後仍走 A17 的 `{t:'act'}` 廣播套用（雙端同 rng 消耗，結果一致，lockstep 不破）。新增訊息：`ask`/`answer`（交易接受拒絕）、`rpsStart`/`rpsMove`/`rpsResult`/`rpsTie`（猜拳）。

**猜拳流程**：攻方（我）出拳先存本地不送；守方出拳 `rpsMove` 送回攻方；攻方兩拳到齊判定（`resolveRPSMoves`），平手送 `rpsTie` 雙方重猜，分勝負送 `rpsResult` 兩邊看揭曉，1.6 秒後攻方 `doAction` 套用＋廣播。攻方的拳到揭曉才公開，避免作弊。

**已連線化的行動**：偷襲（RAID）、建築互換（SWAP_BUILDING）、交易＋被拒猜拳搶（TRADE，含 A13）、強制換原料卡（FORCE_SWAP_RAW——對方原料卡由發起方盲選、不需對方輸入，等同一般廣播動作）。**仍停用**：向玩家購卡（BUY_FROM_PLAYER，含對方喊價協商，最複雜，留待下一版）。

**已驗證**：單機對電腦的本地猜拳/交易流程無回歸；連線猜拳（pick/reveal）與交易回應 modal 渲染正常。**待實機**：真正兩裝置跑完整互動同步（單分頁 harness 無法同時跑兩份 app）。
