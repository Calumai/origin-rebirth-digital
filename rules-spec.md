...
## A19：改「家屋數量決勝」＋手牌全部攤開（2026-07-11 JJ 決議，數位版新規則）

**背景**：JJ 反應現在玩法「有點奇怪，應該只要有一個單一目標」，指定以蓋建築物為主線；並要求「每個人的手牌都要顯示」。查現況：蓋滿本族 4 棟家屋只是**結束觸發**，真正贏家是**總分最高**（家屋＋文化卡＋工藝＋服飾＋獎勵），目標與勝負不一致；手牌只顯示當前玩家自己（A12 隱藏資訊設計）。此為**偏離正式規則書積分制**的數位版改動，已請 JJ 二選一拍板。

**決議內容**：
1. **決勝改為家屋數量**：遊戲結束時，**本族家屋棟數最多者獲勝**，平手才比總分。結束觸發條件不變（有人蓋滿本族 4 棟，或建築牌庫抽空）。文化卡/工藝/服飾從「獨立贏法」降級為「平手加分＋加速手段」，保留策略深度但主線單一。
2. **手牌全部攤開**：所有玩家手牌一律正面朝上（含連線對戰，JJ 選「全部攤開卡面」）。

**平衡分析（game-designer 角度）**：
- 每族 4 棟家屋、全牌庫 16 棟、只有本族計分 → 完成 4 棟＝獨佔該族全部＝保證奪冠（等同競速），抽空牌庫的殘局由家屋數多者勝。
- Bot 現有優先序（`bot.js`：有 4 種素材就優先 `BUY_BUILDING`，註解「符合主勝利路線」）本就以蓋家屋為第一優先，決勝改家屋數後 Bot 策略反而更貼合，**無需調整 AI**。
- 未跑 `simulate.js`（JJ 偏好不自動壓測）；引擎合法動作與 rng 路徑完全不變，只改「計分呈現」與「勝負判定依據」，M1/A11 完賽性不受影響。

**實作**：
- `scoring.js` `scorePlayer` 新增 `detail.buildingCount`（本族家屋棟數）。
- `app.js` 新增 `rankCmp`（家屋數為主、總分為輔）、`isLeader`、`openHandRow`；`versusStripMulti`/`versusStripDuel` 改顯示「家屋 X/4」為主、總分次要、領先改用 `isLeader`，並在每位玩家格加攤開手牌列；`renderEnd` 冠軍與排名改依 `rankCmp`，表格加「家屋棟數」欄；開場說明文字改為「本族家屋蓋最多的人獲勝」。
- `style.css` 加 `.vs-hand`/`.vs-hand-card` 樣式。
- 隱私取捨：A12 的原料盲選、A18 的連線隱藏資訊在「手牌全開」下失去意義（JJ 已知並選擇全開）。

### A19 追加（2026-07-11 JJ 回饋後續）

JJ 試玩後點出三個問題，決議如下：

**1. 建築卡改「每族專屬」**：原本 16 棟建築（每族 4）洗成同一副共用牌庫、隨機抽，會抽到別族的（不算分），牌庫抽光時建築散落各家、誰都湊不滿本族 4 棟。JJ 選「每族專屬」。
- 實作（最小改動）：`actions.js` `drawBuilding` 改為 `state.buildingDeck.findIndex(b => b.tribe === p.tribe)` 只抽本族家屋（`buildingDeck` 結構不動）。每族 4 棟保證留給該族，人人湊得滿，變公平競速。`bot.js` 買建築的 gate 改判本族仍有存量；UI 建築牌庫數字改顯示「本族可蓋家屋 X」。
- 連帶：`SWAP_BUILDING`（建築互換猜拳）在每族專屬下，對方手上一律是「對方本族」建築，猜拳勝＝搶走對方一棟（害他退一步、對自己不算分），仍為有效破壞行動，程式邏輯不變（`want` 找不到本族 → fallback 取第一棟）。
- 結束條件：主觸發仍是「蓋滿本族 4 棟」（`checkBuildingSetWin`），per-tribe 後更可靠（人人抽得到自己的）；2-3 人局 `buildingDeck` 不會抽空（缺席族的建築留著），但 set-completion 一定會結束遊戲，故 `building_deck_empty`／`decks_exhausted` fallback 在 2-3 人局不觸發亦無妨。決勝並列（多人同回合蓋滿）時比總分，與「家屋數決勝」一致。

**2. 擲骰上限 4→3**：JJ 反應「骰到 6 點一次能做太多事」。`actions.js` `rollTurnDice` 由 `d<=2?2:d<=4?3:4` 改為 `d<=2?2:3`（1-2 點→2、3-6 點→3；期望值由 3 降為 2.67，上限 3）。`turnStartAP` 連動，Bot 回合開始判定不受影響。

**3. 猜拳畫面美化**：出拳三鍵改等寬（各 104px、icon 上字下）；揭曉畫面由純文字改為雙方 icon 並排「你 VS 對手」、勝方發光框（`renderRPS` reveal + `.rps-reveal*` CSS）。

## A20：公共事件卡 + 秘密目標（2026-07-11 JJ 工作單）

**背景**：玩家易變成一直抽牌存素材，回合間缺變化、缺短/長期目標。JJ 下工作單加兩套系統（不重做戰鬥/牌組/連線）。

**公共事件卡（6 張）**：開局翻第 1 張，每輪回到第 1 位玩家翻下一張，效果持續到下一張翻開；順序由 seed 決定（`eventDeck` 洗牌 + 純索引推進，不消耗遊戲 rng → 連線 lockstep 一致）。6 張：豐收季（首次拿素材 +1 盛產）、文化祭典（打文化卡 +1 分）、山路封閉（禁交易）、工藝競賽（首位完工藝 +2）、家屋加固（蓋家屋 -1 AP，最低 1）、夜間集會（首次互動成功抽 1 文化）。全部真的生效，無 TODO 假效果。

**秘密目標（6 張）**：開局每人依 seed 發 1 張（Bot 也有），完成 +5 分。家屋守護者/文化傳承者/工藝大師/部落商人/素材收藏家/跨族交流。隱私：seed 兩端各自算出、**不進網路訊息**；UI 只顯示檢視者自己那張（`myPlayer()`），結算才全揭曉。進度：多數由結束盤面推得，只有 deals（交易/購卡）與 partners（互動玩家）需執行期累計（`p.progress`）。

**計分**：`scorePlayer` 加 `objective`（完成 +5）與 `eventBonus`（文化祭典/工藝競賽累計於 `p.bonusScore`），計入 total；決勝仍以 A19 家屋棟數為主、total 為輔。

**驗證**：`node --check app.js` OK；`node game-engine/simulate.js 1000` → 1000/1000（2/3/4 人）；node 決定性測試確認同 seed 事件序列與目標指派一致。詳見 `CLAUDE_GAMEPLAY_CHANGE_NOTE.md`。

## A22：緊急狀況——文化問答 PK（2026-07-12 JJ 構想）

**背景**：JJ 提出「時不時跳出一張卡片說有緊急狀況，玩家可以 PK 族語單詞、或遊戲中的傳說故事題目」。

**決議內容**：
- 真人回合擲骰後約 25% 機率（至少隔 2 個真人回合）跳出「⚡緊急狀況」問答卡：4 選 1 文化題。
- **PK**：答對＝自己 +1 缺少的素材；答錯＝下一位對手「搶答成功」+1 隨機素材。不消耗行動點（額外插入，不動 AP 經濟）。
- **平衡**：EV ≈ 每 4 個真人回合 1 枚素材差距，輕微、不動搖家屋競速主線；獎勵走素材而非分數，不改 scoring。
- **題庫**：全部由 `cards.js` 既有卡片動態生成、內容保證正確——16 題傳說歸屬（myth 卡名「族名-傳說名」反出題）＋4 題工藝歸屬＋4 題食物歸屬＝24 題，同局不重複直到出完。干擾選項：工藝/食物題用其餘 3 個玩家族（順便讓小孩記住場上族群）；傳說題用其他傳說族名。
- **族語單詞題**：`LANG_QUIZ` 留空的資料結構＋格式註解，**待 JJ 提供正式族語內容**（延續待補區「族語會話 P.29–P.44」；不自行杜撰族語）。
- **lockstep 安全**：觸發與選項洗牌用 UI 層 `Math.random`，完全不碰遊戲 rng → simulate／既有壓測不受影響；**連線版 v1 不觸發**（雙端同步協定留待之後，TODO）。

**實作**：`app.js` `buildQuizPool`／`maybeTriggerQuiz(force)`（掛在 `diceDone` 後；`force` 供測試/展示）／`quizPick`／`renderQuiz`；`style.css` `.quiz-*`。教學或其他 modal 開啟時不打斷。
...
## A23：公共事件擴充為 14 張（2026-07-17 使用者回饋）

**背景**：使用者反映回合事件偏少，希望每輪變化更明顯。

**決議**：保留 A20 原有 6 張並新增 8 張：交流市集、互助工班、共學之夜、天候放晴、雨勢觀察、材料試驗、工藝共學、聚落修繕。事件總數固定為 14，於 `cards.js` 定義並由既有每輪翻牌流程驅動。

**效果原則**：
- 事件必須有 `category` 與 `desc`，UI 顯示牌序及「本輪」時效。
- 一次性效果在擲骰開始時重置旗標，不能跨回合重複領取。
- `clearweather` 讓低骰值也取得 3 AP；`materiallab` 首次原料抽牌免費但仍須有可行動狀態；其餘效果詳見 `game-engine/actions.js`。
- 所有新增事件使用既有 seed／牌序，不新增 UI 層隨機判定。

## A24：正向互動與互助分（2026-07-17 使用者回饋）

**分享素材 `SHARE_MATERIAL`**：支付 1 AP，指定另一位玩家與一種自己至少持有 2 枚的素材，轉移 1 枚；每回合一次。發起者取得互助分，總上限 5；互助工班事件可加成。雙方都記錄對方為互動夥伴。

**共學交流 `SHARED_LEARNING`**：支付 1 AP，指定另一位玩家，依固定順序讓雙方各抽 1 張文化卡；文化牌庫少於 2 張時不可用，每回合一次。共學之夜事件下首次使用退回 1 AP。此行動不給基礎互助分，避免同時拿牌又刷分。

**Bot 與同步**：兩個動作都不需要對方隨機輸入；Bot 只在有合理盈餘、牌庫或事件條件時發起。決勝仍依 A19 家屋棟數優先、總分為輔。

## A25：STEAM 教案不屬於遊戲規則真相（2026-07-17 使用者回饋）

`steam-lessons.js` 是獨立教學產生器，不修改遊戲 RNG、牌庫、回合或計分。相同學段、主題與 seed 必須產生相同教案。

建築卡與素材卡只標記為 `game-artifact`；學生因果解釋標記為 `student-hypothesis`；氣候背景需指向登錄官方來源。任何特定族群的家屋、材料與氣候適應敘述，在具備來源、審校者、日期與 `approved` 狀態前，只能是 `review-required`，不得進入已核准隨機內容池。
