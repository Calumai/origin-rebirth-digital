 ...
 *   1. 主選單 menu：部落晨霧感——木琴、口簧琴、低音風聲，緩慢冥想。
 *   2. 選族 setup：稍有節奏的準備音樂；每選一族給不同敲擊音色＋確認音。
...
(function () {
  ...
  // 木頭敲擊（Woodblock）：短促帶通噪音 + 一點高音撥點（邵族音色）
  ...
  // 竹管敲擊（Bamboo clack）：兩根竹子相擊，比木頭清脆、音高偏高（噶瑪蘭音色）
  ...
  // 石頭敲擊（Stone）：低沉、乾、短，帶悶悶的重量（拉阿魯哇音色／山路封閉）
  ...
  // 戰鼓（Drum）：比 thump 更緊實、帶敲擊質感（賽德克音色／偷襲／競賽）
  ...
  function scene(name) {
    if (name === currentScene && sceneTimer) return; // 同場景已在播，不重起
    currentScene = name;
    sceneToken++;
    clearSceneTimer();
    stopTextures();
    if (muted || !ctx) return;                // 尚未解鎖／靜音：記著就好
    resume();
    if (name === 'none' || !SCENES[name]) return;
  ...
  // ── 選族確認音色（每族不同敲擊 + 共同確認音）──────────
  var TRIBE_SFX = {
    thao: function (s) { woodblock(s, 0.11, 820); },      // 邵族：木頭
    kavalan: function (s) { bamboo(s, 0.09); },           // 噶瑪蘭族：竹子
    hlaalua: function (s) { stone(s, 0.11); },            // 拉阿魯哇族：石頭
    seediq: function (s) { drum(s, 0.13); }               // 賽德克族：鼓聲
  };
  function tribe(id) {
    if (muted || !ctx) return; resume();
    var s = t0();
    (TRIBE_SFX[id] || function () {})(s);
    // 共同確認音：上行小三度木琴，「選定了」
  ...
  function toggleMute() {
    ...
    if (muted) { sceneToken++; clearSceneTimer(); stopTextures(); }
    else { ensureCtx(); resume(); restartCurrentScene(); }
    updateBtn();
    return muted;
  }
  function makeToggle() {
  ...
  // 第一次互動：解鎖音訊 + 起目前場景配樂（若未靜音）
  function unlock() {
    ensureCtx(); resume();
    if (!muted) restartCurrentScene();
  }
  ...
  // startAmbient 舊名保留：確保目前場景在播（預設 board，向後相容）
  function startAmbient() { if (!muted) { ensureCtx(); resume(); restartCurrentScene(); } }
